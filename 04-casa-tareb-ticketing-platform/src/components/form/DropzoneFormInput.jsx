      ...
      <Dropzone onDrop={acceptedFiles => handleAcceptedFiles(acceptedFiles, onFileUpload)} maxFiles={5}>
        ...
        getInputProps
            ...
            {showPreview && selectedFiles.length > 0 && <div className="dz-preview row g-4">
                {(selectedFiles || []).map((file, idx) => <Col xl={2} md={4} sm={6} key={`file-${idx}-${file.name}`}>
                    <div className="card p-2 mb-0 shadow-none border position-relative h-100">
                      {file.preview ? <img alt={file.name} src={file.preview} className="rounded bg-light" /> : <div className="rounded bg-light text-center">{file.name.split('.')[file.name.split('.').length - 1]}</div>}
                      <div className="mt-2">
