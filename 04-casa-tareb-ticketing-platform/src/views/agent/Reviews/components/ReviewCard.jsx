  ...
  return <div className="bg-light rounded p-3">
      <div className="d-sm-flex justify-content-between">
        ...
        <ul className="list-inline mb-2 mb-sm-0">
          {Array.from(new Array(Math.floor(review.rating))).map((_star, idx) => <li key={idx} className="list-inline-item me-1">
              <FaStar size={18} className="text-warning" />
            </li>)}
          {!Number.isInteger(review.rating) && <li className="list-inline-item me-1">
              <FaStarHalfAlt size={18} className="text-warning" />
            </li>}
          {review.rating < 5 && Array.from(new Array(5 - Math.ceil(review.rating))).map((_val, idx) => <li key={idx} className="list-inline-item me-1">
                <FaStar size={18} />
              </li>)}
        </ul>
      </div>
      ...
      {review.images && <Row className="g-4">
          {review.images.map((image, idx) => {
        return <Col key={idx} xs={4} sm={3} lg={2}>
