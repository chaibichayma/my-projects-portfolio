              ...
              <Row key={idx}>
                <Col sm={8}>
                  ...
                  <ul className="list-inline mb-0">
                    {Array.from(new Array(Math.floor(hotel.rating))).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small">
                        <FaStar size={16} className="text-warning" />
                      </li>)}
                    {!Number.isInteger(hotel.rating) && <li className="list-inline-item me-1 small">
                        <FaStarHalfAlt size={16} className="text-warning" />
                      </li>}
                    {hotel.rating < 5 && Array.from(new Array(5 - Math.ceil(hotel.rating))).map((_val, idx) => <li key={idx} className="list-inline-item me-1 small">
                          <FaStar size={16} />
                        </li>)}
                  </ul>
                </Col>
